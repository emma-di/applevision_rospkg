...
class EnvProperties:
    def __init__(self, delta_t_ms, accel_std, starting_position, starting_std, z_std, backdrop_dist, apple_r, dist_fov_rad, camera_info):
...
class KalmanMatricies:
    ...
    def __init__(self, F, G, Q, H, I):#F: np.ndarray, G: np.ndarray, Q: np.ndarray, H: np.ndarray, I: np.ndarray) -> None:
        self.F = F
        self.G = G
        self.Q = Q
        self.H = H
    ...
    @staticmethod
    def _upscale_dim(mtx, ndim): #:np.ndarray, ndim: int) -> np.ndarray:
        assert ndim > 0
...
class KalmanFilter():
    ...
    def __init__(self, env, std_range, too_close_cam, too_far_dist, appl_proportion_low, appl_proportion_high): #env: EnvProperties, std_range: float, too_close_cam: float,
    ...
    @staticmethod
    def _calc_per_apple_in_fov(env, pos): #:EnvProperties, pos: Vec3) -> float:
    ...
    @staticmethod
    def _compute_camera_from_bounding_box(env, box): #EnvProperties, box: RegionOfInterestWithConfidenceStamped) -> Tuple[Vec3, Vec3]:
    ...
    def _compute_dist_var(self, cam_pos, cam_var, dist_meas, est_pos): #Vec3, cam_var: Vec3, dist_meas: float, est_pos: Vec3) -> float:
        mx, my, mzc = cam_pos
        mzd = dist_meas
    ...
    def step_filter(self, box, dist, control): #: Optional[RegionOfInterestWithConfidenceStamped], dist: Optional[Range], control: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
#         return np.transpose(x_est_ee)[0], self.p_est, 
