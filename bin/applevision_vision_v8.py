...
from applevision_rospkg.bin.applevision_vision1 import MODEL_PATH
from mss import mss
...
#     sct = mss()
#     monitor = {'top': 360, 'left': 640, 'width': 640, 'height': 360}

#     img = Image.frombytes('RGB', (640, 360), sct.grab(monitor).rgb)
...
def unwrap_detection(input_image, output_data):
    ...
    output_data = np.reshape(output_data, (1200, 6))
    rows = output_data.shape[0]
    
    # rospy.logwarn(str(output_data.shape))
    ...
    for r in range(rows):
        row = output_data[r]
        # rospy.logwarn(str(row.shape))
        confidence = 1/(1+np.exp(-row[4]))
        # confidence = model(input_image)
        # rospy.logwarn(str(confidence))
        if confidence >= CONFIDENCE_THRESH:
            rospy.logwarn(str(confidence))
            classes_scores = row[5:]
            _, _, _, max_indx = cv2.minMaxLoc(classes_scores)
            class_id = max_indx[1]
            rospy.logwarn(str(classes_scores))
            if (classes_scores[class_id] > .25):

                confidences.append(confidence)
                class_ids.append(class_id)

                x, y, w, h = 1/(1+np.exp(-row[0])),1/(1+np.exp(-row[1])),1/(1+np.exp(-row[2])),1/(1+np.exp(-row[3]))
                ...
                thing1 = 'full_row with high enough confidence and score' + str(box)
                rospy.logwarn(thing1)
                boxes.append(box)
...
class AppleVisionHandler:
    ...
    def run_applevision(self, im: Image):
        ...
        rospy.logwarn(str(adjusted_image.shape))
        
        # adjusted_image = np.reshape(adjusted_image,(1,640,640,3))
        self.net.setInput(adjusted_image)
        predictions = self.net.forward()
        # rospy.logwarn('python version'+sys.version)

        # rospy.logwarn(str(predictions.shape))
        output = predictions[0]
        # rospy.logwarn(str(type(predictions[0])))
